...
zstyle :compinstall filename '/Users/pivotal/.zshrc'
...
HASH_NUM=$(echo $HOSTNAME | shasum | tr -d 'a-f' | cut -b 1-6)
...
precmd(){
    ...
    # git-duet
    if [[ "$duet_pair" != "" ]]; then
        DUET=" %B%F{green}*%f%b%B%F{$HASH_MOD} [%B%F{green}${duet_pair}%b%f%B%F{$HASH_MOD}]%b%f"
    else
        DUET=""
    fi
